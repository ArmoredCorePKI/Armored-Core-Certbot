...
def prepare_and_parse_args(plugins: plugins_disco.PluginsRegistry, args: List[str]
    ...
    helpful.add(
        "acorefunc", "--verify-puf-cert", action='store_true',
        default=flag_default("verify_puf_cert"), dest="verify_puf_cert",
        help="Verify the PUF-based certificates in Armored Core.")

    helpful.add(
        "acorefunc", "--test-domain", action='store', type=str,
        default=flag_default("test_domain"), dest="test_domain",
        help="Test domain for certificate verification in Armored Core.")
