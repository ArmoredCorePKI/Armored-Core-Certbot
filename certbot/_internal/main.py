...
from certbot.acore import acore_verify
from certbot.acore import acore_utils
from certbot.trillianclient import tclient_util
...
# We only conside certonly mode for now, i.e., not actually installing the requested certificates
...
def acore_func(config: configuration.NamespaceConfig, plugins: plugins_disco.PluginsRegistry)->None:
    """Armored Core functions including certificate verification

    This implements the 'certonly' subcommand. This function is relatively independent of other components in certbot such as ACME client.
    It will introduce several standalone verification function that can be ported anywhere else.
    ...
    """
    #print("In acore_func")
    #print("test domain", config.verify_puf_cert, config.test_domain, config.config_dir)

    if config.verify_puf_cert is False:
        print("No actions specified.")
        return

    start_t1 = time.time()
    tr = tclient_util.TrillianClient()
    raw_entry_list = tr.get_entry_list()
    end_t1 = time.time()
    print("Measuring 1: ", end_t1 - start_t1)

    start_t2 = time.time()
    entry_list = tclient.construct_entry_list(raw_entry_list)

    # acquire the cerificates from config_dir
    raw_certs = acore_utils.read_certs_from_path(config.config_dir, config.test_domain)
    cert_list = acore_utils.parse_pem_certs(raw_certs)
    end_t1 = time.time()

    ret = acore_verify.verify_all(cert_list, entry_list)
    end_t2 = time.time()
    print("Measuring 2: ", end_t2 - start_t2)
    if ret is False:
        print("Verify Failed!")
        return
    else:
...
def main(cli_args: Optional[List[str]] = None) -> Optional[Union[str, int]]:
    ...
    with make_displayer(config) as displayer:
        ...
        return config.func(config, plugins) # certonly reflected into self.verbs and config.func, consequently.
